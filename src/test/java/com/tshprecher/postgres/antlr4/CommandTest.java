	...
	public void test(String commandName, String testDir) throws IOException {
		...
		try {
			...
			for (File fileUnderTest : testFiles) {
				String name = fileUnderTest.getPath().substring(fileUnderTest.getPath().lastIndexOf(File.separatorChar));
				try (FileInputStream fio = new FileInputStream(fileUnderTest)) {
					...
					if (success.get() != expect) {
						System.out.printf("failed: %s%n%s%n", name,FileUtils.readFileToString(fileUnderTest, Charset.defaultCharset()));
						if (expect)
							falseNegative++;
						else
							falsePositive++;
			...
			throw new RuntimeException("Unexpected when reading uri", e);
		}
	}
